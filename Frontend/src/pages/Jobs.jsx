        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {searchedJobs.map((job, index) => (
              <Recommendations
                key={index}
                jobTitle={searchedJobs[index].Title} // Map jobTitle from backend response
                position={searchedJobs[index].Position} // Map position from backend response
                company={searchedJobs[index].Company} // Map company from backend response
                jobDescription={searchedJobs[index].JobDescription} // Map jobDescription from backend response
                status={searchedJobs[index].Status} // Map status (Open/Closed) from backend response
              />
            ))}
          </div>
        </div>
